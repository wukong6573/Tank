 ...
 */
public class Water extends Pictrue {
...
//		this.y = y;
		...
		//工具类绘制
		try {
			int[] size = DrawUtils.getSize(Bullet.class.getResource("/res/img/water.gif").getPath());
			this.width = size[0];
			this.height = size[1];
		} catch (IOException e) {
			// TODO 自动生成的 catch 块
	...
	public void draw(){
		...
		try {
			DrawUtils.draw(Bullet.class.getResource("/res/img/water.gif").getPath(), x, y);//0,64
		} catch (IOException e) {
			// TODO 自动生成的 catch 块
			e.printStackTrace();
